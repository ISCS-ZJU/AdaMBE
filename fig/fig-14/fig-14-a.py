...
c = canvas(ctype, 'fig-14-a', dimensions=[400, 300])

t = table(file='fig-14-a.data')

# print(min_value)


d = drawable(canvas=c, xrange=[3, 120],
             yrange=[20, 150000], dimensions=[pictureWidthMini, pictureHeightMini], coord=[40,30], yscale='log10', xscale='log2')
...
#c.box(coord=[[d.left()+7, d.top()-55], [d.left()+75, d.top()-35]], fill=True, fillcolor="white", linewidth=0.5)
L.draw(canvas=c, coord=[d.left()+13, d.top()-40], width=5, height=5, vskip=5, fontsize=legendTextSize, skipnext=2, skipspace=74, order=[1,0])
L_line.draw(canvas=c, coord=[d.left()+10, d.top()-40], width=10, height=10, vskip=0, fontsize=legendTextSize, skipnext=2, skipspace=74, order=[1,0])
