...
c = canvas(ctype, title='fig-8-b', dimensions=[700, 150])


######################################real_time#################################

t = table(file='fig-8-b.data')
d = drawable(canvas=c, xrange=[-0.6, t.getmax('rownumber')+0.6],
             yrange=[2, 50000], dimensions=[pictureWidthLong, pictureHeightLong], coord=[30,30], yscale='log10')
...
axis(drawable=d, style='box', ticstyle='in',
         ...
         ytitle='Memory usage (MB)', doylabels=True,ytitleshift=[ytitleShiftX,ytitleShiftY],
         linewidth=0.8,
         #yaxisposition=1,
         #xaxisposition=0, yauto=['','',3],
#          xlabelrotate=20,
        xaxisposition=2 * 0.75, #yauto=['','',0.5], 
        # xlabelshift=[0, -1],
...
series_list = ['AdaMBEFinder',	'FmbeFinder', 'PmbeFinder',	'ooMBEAFinder', 'ParAdaMBEFinder',	'ParMBE',	'GMBE']
series_name = ['AdaMBE','FMBE','PMBE','ooMBEA','ParAdaMBE','ParMBE','GMBE']
bgcolors    = ['white', 'white', 'white','white', fig_colors[0], fig_colors[1], fig_colors[4]]
fillcolors  = fig_colors[:4] + ['black'] * 3
...
for i in range(len(series_list)):
    p.verticalbars(drawable=d, table=t, xfield='rownumber', yfield=series_list[i],
               ...
               fillstyle=bartypes[i][0], fillsize=bartypes[i][1], fillskip=bartypes[i][2])
...
#### legend
L.draw(canvas=c, coord=[d.left()+1, d.top()+10], skipnext=1, skipspace=85,
